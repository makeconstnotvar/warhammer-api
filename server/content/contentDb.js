...
function toNumberArray(values) {
  ...
  return values.map((value) => toNumber(value)).filter((value) => value !== null);
}
...
const resourceDbConfigs = {
  ...
  "warp-routes": {
    ...
    filterBuilders: {
      ...
      fromStarSystem: (context, rawValue) =>
        addRelationFilter(
          context,
          rawValue,
          (placeholder) => `EXISTS (
    ...
    serialize(row) {
      ...
      return {
        ...
        fromStarSystemId,
        toStarSystemId,
        starSystemIds: [fromStarSystemId, toStarSystemId].filter(
  ...
  fleets: {
    ...
    filterBuilders: {
      ...
      currentStarSystem: (context, rawValue) =>
        addRelationFilter(
          context,
          rawValue,
          (placeholder) => `EXISTS (
  ...
  campaigns: {
    ...
    filterBuilders: {
      ...
      organizations: (context, rawValue) =>
        addRelationFilter(
          context,
          rawValue,
          (placeholder) => `EXISTS (
...
async function searchResourceRows(resourceKey, search, limit) {
  ...
  const searchRankExpression = `
    ...
    END
  `;
  const fallbackOrderClause = buildOrderClause(config, getDefaultSearchSort(config)).replace(
    /^ORDER BY\s+/i,
    ""
  );
  const dataQuery = `
