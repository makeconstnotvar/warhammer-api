...
function createDecorator(fetchData, defaultData = [], options = {}) {
  const BaseStoreClass = class {
    ...
    fetchDataAdapter =
      options.fetchDataAdapter ||
      function (response) {
        return response;
      };
    fetchMethod =
      options.fetchMethod ||
      function (_params, _executor) {
        throw new Error("Не задан fetchMethod");
      };
    fetchFailed = options.fetchFailed || function (_error) {};
    fetchSuccess = options.fetchSuccess || function (_response) {};
    checkPermissions = options.checkPermissions || function (_error) {};
    checkNavigation = options.checkNavigation || function (_error) {};
    logError =
      options.logError ||
      function (error) {
        console.log(error);
