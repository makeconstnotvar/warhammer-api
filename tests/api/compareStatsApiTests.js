  ...
  const cases = [
    ...
    },
    {
      ...
      run: async () => {
        ...
        assert.deepEqual(json.data.comparison.segmentums.sort(), [
          "Segmentum Solar",
          "Ultima Segmentum",
        ]);
        assert.deepEqual(Object.keys(json.included).sort(), [
    ...
    {
      name: "compare keeps items alias compatible with ids parsing",
      run: async () => {
        const { json, response } = await getJson(
          baseUrl,
    ...
    {
      name: "compare returns validation error when fewer than two identifiers are requested",
      run: async () => {
        const { json, response } = await getJson(
          baseUrl,
        ...
        assert.deepEqual(json.error.details, [
          {
            ...
            count: 1,
            field: "ids",
            message: 'Parameter "ids" must contain at least two identifiers',
            minItems: 2,
            value: "imperium-of-man",
          },
        ]);
      },
    },
        ...
        assert.deepEqual(json.error.details, [
          {
            ...
            resource: "factions",
            value: "imperium-of-man,missing-record",
          },
        ]);
      },
    },
