...
- Legacy `/api` не является целевым состоянием продукта и уже выведен из публичного runtime-контракта.
...
Консолидация к одному API уже закреплена в runtime и docs:

1. `/api/v1` является единственным публичным контрактом.
2. Docs/navigation/onboarding описывают только `/api/v1`.
3. Legacy CRUD-слой и отдельные legacy docs удалены из активного дерева проекта.
4. Дальнейшая разработка идет только вокруг единого контракта `/api/v1`.
